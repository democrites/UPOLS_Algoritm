...
// typedef enum {
//     O_RDONLY    = 0,        // Opens the file in read-only mode.
//     O_WRONLY    = (1U << 0),// Opens the file in write-only mode.
//     O_RDWR      = (1U << 1),// Opens the file in read and write mode.
//     O_CREAT     = (1U << 2),// Create a file if it doesn’t exist.
//     O_EXCL      = (1U << 3),// Prevent creation if it already exists.
//     O_APPEND    = (1U << 4),// Opens the file and places the cursor at the end of the contents.
//     O_ASYNC     = (1U << 5),// Enable input and output control by signal.
//     O_CLOEXEC   = (1U << 6),// Enable close-on-exec mode on the open file.
//     O_NONBLOCK  = (1U << 7),// Disables blocking of the file opened.
//     O_TMPFILE   = (1U << 8) // Create an unnamed temporary file at the specified path.
// } open_flags_t;

// // Define bitwise flags for each mode
// #define O_RDONLY        (0)         // Read-only mode
// #define O_WRONLY        (1U << 0)   // Write-only mode
// #define O_RDWR          (1U << 1)   // Read and write mode
// #define O_CREAT         (1U << 2)   // Create file if it doesn’t exist
// #define O_EXCL          (1U << 3)   // Exclusive creation, fail if exists
// #define O_APPEND        (1U << 4)   // Append mode, place cursor at the end
// #define O_ASYNC         (1U << 5)   // Enable asynchronous I/O via signals
// #define O_CLOEXEC       (1U << 6)   // Close on exec mode
// #define O_NONBLOCK      (1U << 7)   // Non-blocking mode
// #define O_TMPFILE       (1U << 8)   // Create an unnamed temporary file
